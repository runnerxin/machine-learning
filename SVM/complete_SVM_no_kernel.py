...
class Svm:
    def __init__(self, data_in, class_labels, c, tolerance):  # Initialize the structure with the parameters
...
def calc_ek(svm, i):
    ...
    f_k = np.multiply(svm.alphas, svm.Y).T * svm.X * svm.X[i].T + svm.b
...
def inner_loop(svm, i):
    ...
    if ((svm.Y[i] * ei < -svm.tolerance) and (svm.alphas[i] < svm.C) or
        ...
        b1 = svm.b - ei - svm.Y[i] * (svm.alphas[i] - alpha_i_old) * svm.X[i] * svm.X[i].T - svm.Y[j] * (
                svm.alphas[j] - alpha_j_old) * svm.X[i] * svm.X[j].T
        b2 = svm.b - ej - svm.Y[i] * (svm.alphas[i] - alpha_i_old) * svm.X[i] * svm.X[j].T - svm.Y[j] * (
                svm.alphas[j] - alpha_j_old) * svm.X[j] * svm.X[j].T
...
def smo_svm(data_in, lable_in, c, tolerance, max_iter):
    ...
    # 创建一个 svm 对象
    svm = Svm(np.mat(data_in), np.mat(lable_in).transpose(), c, tolerance)
    epoch = 0
    entire_set = True
    alpha_changed = 0
